...
Generated from mqobsidian agent-entrypoint templates for mq-mcp.
...
Regenerate with:
  MQ_OBSIDIAN_DIR=<path-to-mqobsidian> \
    python3 "$MQ_OBSIDIAN_DIR"/scripts/generate-agents-md.py --repo mq-mcp --out AGENTS.md
-->
...
4. Read `${MQ_OBSIDIAN_DIR:-$HOME/mqobsidian}/memory/learn/agent/mq-mcp.md` if it exists.
5. Read `${MQ_OBSIDIAN_DIR:-$HOME/mqobsidian}/systems/mq-mcp/hot.md` if it exists.
6. Read `${MQ_OBSIDIAN_DIR:-$HOME/mqobsidian}/systems/mq-mcp/index.md` if it exists.
7. Read `${MQ_OBSIDIAN_DIR:-$HOME/mqobsidian}/memory/learn/repos/mq-mcp.md` if it exists.
...
uv --directory ${MQ_OBSIDIAN_DIR:-$HOME/mqobsidian}/memory run python commands/emit_observation.py \
  --producer codex --repository mq-mcp --workflow task-work --category pattern --confidence 0.70 \
  ...
  --evidence-excerpt "Concrete evidence summary" --tag mq-mcp
```
