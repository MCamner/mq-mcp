...
| `analyze_guitar_pro` | server.py | `analyze_guitar_pro` | repo files | read-only | Parses GP3/GP4/GP5 files |
| `open_in_app` | server.py | `open_in_app` | shell | execution | Opens file with macOS `open` command |
| `edit_image` | server.py | `edit_image` | repo files | **write** | rotate, grayscale — overwrites in place |
...
`analyze_guitar_pro`, `open_in_app`, and `edit_image` use `os.path.normpath(os.path.join(REPO_ROOT, relative_path))` — safe from traversal but does not raise explicitly.
