...
`--forget` removes the selected date from all three session surfaces. It does not touch learning, reviews, semantic memory, mqobsidian, or the project pin.

## Verification

`tests/test_bridget_context.py` asserts the exact files created by normal session recording, redaction across all stores, bounded injection, and date-scoped deletion. `tests/test_bridge_refactor.py` asserts that context-only tools do not produce a learn suggestion and that learning writes suppress further suggestions.
