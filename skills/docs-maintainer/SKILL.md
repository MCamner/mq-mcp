...
* Product positioning or README launch polish — use the `repo_signal_positioning` and `repo_signal_suggest` MCP tools

## Evals

### Should trigger

* "the README tool count is stale"
* "sync docs after the MCP tool surface change"
* "update the changelog and install docs for the release bump"
* "docs/integration.md mentions behavior that no longer exists"

### Should not trigger

* "change how update_repo_file validates paths" → use `mcp-tool-safety-maintainer`
